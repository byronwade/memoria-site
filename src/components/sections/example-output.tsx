  ...
  return (
    <Section background="muted">
      ...
      >
        ...
        >
          When you ask your AI to analyze a file, Memoria returns actionable intelligence
        </motion.p>
      </motion.div>
      ...
      >
        <Card className="bg-gray-900 overflow-hidden shadow-xl border-gray-800 p-0">
          <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-800">
            ...
            <span className="text-gray-500 text-xs font-mono ml-2">memoria output</span>
          </div>
          <div className="p-6 text-left font-mono text-sm overflow-x-auto">
            <pre className="whitespace-pre-wrap text-gray-300 leading-relaxed">
              {exampleOutput.split('\n').map((line, i) => {
                ...
                if (line.startsWith('###')) {
                  return <div key={i} className="text-white font-bold text-base mb-2">{line.replace('### ', '')}</div>;
                }
                if (line.startsWith('**🔥')) {
                  return <div key={i} className="text-red-400 font-semibold">{line.replace(/\*\*/g, '')}</div>;
                }
                if (line.startsWith('**🔗') || line.startsWith('**🧱') || line.startsWith('**🛑')) {
                  return <div key={i} className="text-emerald-400 font-semibold mt-4">{line.replace(/\*\*/g, '')}</div>;
                }
                if (line.startsWith('**📐')) {
                  return <div key={i} className="text-blue-400">{line.replace(/\*\*/g, '')}</div>;
                }
                if (line.startsWith('**Risk')) {
                  return <div key={i} className="text-amber-400 text-xs mt-1">{line.replace(/\*\*/g, '')}</div>;
                }
                if (line.startsWith('>')) {
                  return <div key={i} className="text-gray-500 italic pl-4 border-l-2 border-gray-700 my-2">{line.replace('> ', '')}</div>;
                }
                if (line.startsWith('- [ ]')) {
                  return <div key={i} className="text-gray-400 pl-4">{line}</div>;
                }
                if (line.startsWith('```')) {
                  return null;
                }
                if (line.startsWith('+')) {
                  return <div key={i} className="text-green-400 pl-4">{line}</div>;
                }
                if (line.startsWith('-') && !line.startsWith('- [') && !line.startsWith('---')) {
