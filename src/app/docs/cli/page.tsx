...
npx @byronwade/memoria analyze src/api/route.ts --verbose`}</code>
      ...
      <p>
        Start the MCP server. This is called automatically by your AI tool&apos;s
        configuration.
      </p>
...
npx @byronwade/memoria analyze --help`}</code>
      ...
      <p>
        For MCP server configuration, npx is recommended as it ensures you always
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td><code>.memoria.json</code></td>
          </tr>
          <tr>
            <td><code>MEMORIA_CACHE_DIR</code></td>
            <td>Cache directory location</td>
            <td><code>.memoria-cache</code></td>
          </tr>
        </tbody>
      </table>
...
if [ "$HIGH_RISK" -gt 0 ]; then
  ...
  exit 1
      ...
      <div className="callout callout-info">
        ...
        <p>
          When running as an MCP server (the default for AI tools), Memoria uses
          the stdio transport and communicates via JSON-RPC. You don&apos;t need to
          interact with the CLI directly in this mode.
        </p>
      </div>
    </div>
  );
